...
export default function SimulatorPage() {
  ...
  const simData = useMemo(() => {
    ...
    for (let year = 1; year <= yearsToSimulate; year++) {
      ...
      // Crypto has high volatility mock factor
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8 relative z-10">
      ...
      <div>
        <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight text-white mb-2 flex items-center gap-3">
          <Landmark className="h-8 w-8 text-pink-400" />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        {/* Controls Panel */}
        <div className="glass-panel p-6 rounded-3xl border border-white/10 space-y-6 lg:col-span-1">
          <h3 className="text-sm font-mono font-bold text-purple-300 uppercase border-b border-white/5 pb-2">
            Simulation Inputs
          </h3>
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between text-xs font-mono mb-1.5">
                <span className="text-gray-400">Starting Age</span>
                <span className="text-white font-bold">{startingAge} yrs</span>
              </div>
              <input
                type="range"
                min="18"
                ...
                className="w-full accent-purple-500 bg-gray-800 rounded-lg cursor-pointer h-1.5"
              />
            </div>

            <div>
              <div className="flex justify-between text-xs font-mono mb-1.5">
                <span className="text-gray-400">Career Starting Salary</span>
                <span className="text-white font-bold">${salary.toLocaleString()}/yr</span>
              </div>
              <input
                type="range"
                min="20000"
          ...
          <h3 className="text-sm font-mono font-bold text-purple-300 uppercase border-b border-white/5 pb-2 pt-2">
            Asset Allocations (%)
          </h3>
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between text-xs font-mono mb-1">
                <span className="text-gray-400">HYSA Cash ({allocCash}%)</span>
                <span className="text-gray-400">Index Stocks ({allocStocks}%)</span>
              </div>
              <div className="flex gap-2">
                <input
                  ...
                  className="w-1/3 bg-black/40 border border-white/10 text-xs rounded-xl p-2 text-white outline-none focus:border-purple-500"
                />
                <input
                  type="number"
                  placeholder="Stocks"
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div>
              <span className="text-[10px] font-mono text-gray-400 block mb-1">Housing</span>
              <select
                value={housing}
                onChange={(e) => setHousing(e.target.value)}
                className="w-full bg-black/50 border border-white/10 rounded-xl p-2 text-white"
              >
                <option value="shared">Shared Room ($600/mo)</option>
                <option value="condo">Condo Rental ($1,800/mo)</option>
                <option value="luxury">Luxury Penthouse ($4,000/mo)</option>
              </select>
            </div>
            <div>
              <span className="text-[10px] font-mono text-gray-400 block mb-1">Transportation</span>
              <select
                value={transit}
                onChange={(e) => setTransit(e.target.value)}
                className="w-full bg-black/50 border border-white/10 rounded-xl p-2 text-white"
              >
                <option value="transit">Metro Card ($80/mo)</option>
                <option value="sedan">Used Sedan ($400/mo)</option>
                <option value="supercar">Cyber Truck lease ($1,500/mo)</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6 flex flex-col justify-between">
          ...
          <div className="glass-panel p-6 rounded-3xl border border-white/10 flex-1 flex flex-col justify-between">
            <h3 className="text-base font-extrabold text-white tracking-wide border-b border-white/5 pb-2 flex justify-between items-center">
              ...
              <span className="text-xs font-mono font-light text-gray-400">40-Year Growth Simulation</span>
            </h3>
            ...
            <div className="h-[260px] w-full mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={simData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <YAxis stroke="rgba(255,255,255,0.4)" fontSize={10} tickFormatter={(v) => `$${v/1000000}M`} />
                  <Tooltip 
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="glass-panel p-5 rounded-2xl border border-white/10 bg-gradient-to-br from-purple-950/20 to-black/50 text-center flex flex-col justify-center">
              <span className="text-[10px] font-mono tracking-widest text-purple-300 uppercase block mb-1">PROJECTED WEALTH</span>
              <div className={`text-xl sm:text-2xl font-black font-mono tracking-wide ${finalNetWorth >= 0 ? "text-emerald-400" : "text-red-400"}`}>
                ${finalNetWorth.toLocaleString()}
              </div>
              <span className="text-[9px] text-gray-500 block uppercase mt-0.5">at Age {startingAge + 40}</span>
            </div>

            <div className="glass-panel p-5 rounded-2xl border border-white/10 bg-gradient-to-br from-pink-950/20 to-black/50 text-center flex flex-col justify-center">
              <span className="text-[10px] font-mono tracking-widest text-pink-300 uppercase block mb-1">OPTIMIZER GRADE</span>
              <div className="text-lg font-black text-white font-mono tracking-wide flex items-center justify-center gap-1">
                {finalNetWorth > 2000000 ? <Sparkles className="h-4.5 w-4.5 text-pink-400 animate-pulse" /> : null}
                {scoreDetails.badge}
              </div>
            </div>

            <div className="glass-panel p-5 rounded-2xl border border-white/10 bg-black/50 sm:col-span-1 flex items-center justify-center">
              <p className="text-[11px] text-gray-300 leading-relaxed italic text-center">
